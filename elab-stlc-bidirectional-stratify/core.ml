...
module Semantics = struct
  ...
  *)
  and nexpr =
    ...
    | Bool_elim of nexpr * thunk * thunk
  ...
  and clos =
    | Clos of vexpr list * expr

  (** A delayed computation, used in the branches of conditionals. *)
  and thunk =
    | Thunk of vexpr list * expr


  (** {1 Evaluation} *)

  (** Evaluate an expression from the syntax into its semantic interpretation *)
  let rec eval (env : vexpr env) (expr : expr) : vexpr =
    match expr with
    ...
    | Bool_elim (head, expr1, expr2) ->
        bool_elim (eval env head) (Thunk (env, expr1)) (Thunk (env, expr2))

  (** Instantiate a closure with a value *)
  and inst_clos (Clos (env, body) : clos) (arg : vexpr) : vexpr =
    eval (arg :: env) body

  (** Force the computation of a thunk *)
  and force_thunk (Thunk (env, body) : thunk) : vexpr =
    eval env body
  ...
  and bool_elim (head : vexpr) (vexpr1 : thunk) (vexpr2 : thunk) : vexpr =
    match head with
    | Neu nexpr -> Neu (Bool_elim (nexpr, vexpr1, vexpr2))
    | Bool_lit true -> force_thunk vexpr1
    | Bool_lit false -> force_thunk vexpr2
...
let pp_name_ann (name : string) (ty : ty) : Format.formatter -> unit =
  Format.dprintf "@[<2>@[%s :@]@ %t@]" name (pp_ty ty)

let pp_param (name : string) (ty : ty) : Format.formatter -> unit =
  Format.dprintf "@[<2>(@[%s :@]@ %t)@]" name (pp_ty ty)
...
let pp_expr : name env -> expr -> Format.formatter -> unit =
  let rec pp_expr names e =
        ...
        let rec go names e =
          ...
          | Let (name, def_ty, def, body) ->
              Format.dprintf "@[<2>@[let %t@ :=@]@ @[%t;@]@]@ %t"
                ...
                (pp_expr names def)
                (go (name :: names) body)
          | e -> Format.dprintf "@[%t@]" (pp_expr names e)
        in
        Format.dprintf "@[<v>%t@]" (go names e)
    | Fun_lit (name, param_ty, body) ->
        let rec go names e =
          match e with
          | Fun_lit (name, param_ty, body) ->
    ...
    | e ->
        pp_app_expr names e
  and pp_app_expr names e =
    let rec go e =
      match e with
      | Fun_app (head, arg) -> Format.dprintf "%t@ %t" (go head) (pp_atomic_expr names arg)
      | e -> pp_atomic_expr names e
    in
    match e with
    | Fun_app _ as e -> Format.dprintf "@[<hv 2>%t@]" (go e)
    | e -> pp_atomic_expr names e
  and pp_atomic_expr names e =
    ...
    | Let _ | Fun_lit _ | Fun_app _ | Bool_elim _ as e ->
        Format.dprintf "@[(%t)@]" (pp_expr names e)
  in
  pp_expr
