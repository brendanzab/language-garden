Eta conversion for functions
  ...
  >   fun A x P px => px;
  > 
  > let tests (A : Type) : Type :=
  >   let F1 : Type := A -> A;
  >   let F2 : Type := A -> A -> A;
  > 
  >   -- Eta-conversion for single argument functions
  >   let test (f : F1) : Eq F1 (fun x => f x) f := refl F1 f;
  >   let test (f : F1) : Eq F1 f (fun x => f x) := refl F1 f;
  > 
  >   -- Eta-conversion for multi-argument functions
  >   let test (f : F2) : Eq F2 (fun x y => f x y) f := refl F2 f;
  >   let test (f : F2) : Eq F2 f (fun x y => f x y) := refl F2 f;
  > 
  >   Type;
  > 
  > Type
  ...
  <stdin> : Type :=
    ...
    let tests : fun (A : Type) -> Type :=
      fun A =>
        let F1 : Type := fun (_ : A) -> A;
        let F2 : Type := fun (_ : A) (_ : A) -> A;
...
Swapping arguments prevents eta conversion
  ...
  >   let F2 : Type := A -> A -> A;
  > 
  >   let test (f : F2) : Eq F2 (fun x y => f y x) f := refl F2 f;
  ...
  error: mismatched types
     ┌─ <stdin>:10:52
     │
  10 │   let test (f : F2) : Eq F2 (fun x y => f y x) f := refl F2 f;
     │                                                     ^^^^^^^^^
