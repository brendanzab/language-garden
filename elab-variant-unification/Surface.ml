...
let rec elab_check (context : context) (tm : tm) (ty : Core.ty) : Core.tm =
  ...
  | Match (head, clauses), body_ty ->
      elab_check_match context head clauses body_ty
...
and elab_infer (context : context) (tm : tm) : Core.tm * Core.ty =
  ...
  | Match (head, clauses) ->
      ...
      elab_check_match context head clauses body_ty, body_ty
...
and elab_check_match (context : context) (head : tm) (clauses : (pattern * tm) list) (body_ty : Core.ty) : Core.tm =
  let head_loc = head.loc in
  let head, head_ty = elab_infer context head in
