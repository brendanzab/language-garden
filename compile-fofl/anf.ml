...
module rec Expr : sig
  ...
  type t =
    ...
    | Join of Join_id.t * (Local_id.t * Ty.t) * Ty.t * t * t
  ...
  let eval (items : Item.t Item_map.t) (expr : t) : value =
    let rec eval (joins : (Local_id.t * t) Join_map.t) (locals : value Local_map.t) (expr : t) : value =
      ...
      | Join (id, (param_id, _), _, cont, body) ->
        ...
        let rec go expr ppf =
          ...
          | Let (id, def_ty, def, body) ->
              Format.fprintf ppf "@[<2>@[let %t@ :=@]@ @[%t;@]@]@ %t"
                ...
                (pp_comp def)
                (go body)
          | Join (id, (param_id, param_ty), cont_ty, cont, body) ->
              Format.fprintf ppf "@[<2>@[join %s@ @[(%s@ :@ %t)@]@ :@ %t@ :=@]@ @[%t;@]@]@ %t"
