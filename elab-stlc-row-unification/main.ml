...
let elab_tm (input : string) (tm : Surface.tm) : Core.tm * Core.ty =
  ...
  | unsolved_metas ->
      unsolved_metas |> List.iter (function
        | (pos, `Fun_param) -> print_error pos input "ambiguous function parameter type"
        | (pos, `Fun_body) -> print_error pos input "ambiguous function return type"
        | (pos, `Record_field) -> print_error pos input "ambiguous record field"
        | (pos, `Match_clauses) -> print_error pos input "ambiguous match clauses"
        | (pos, `Pattern_binder) -> print_error pos input "ambiguous pattern binder"
