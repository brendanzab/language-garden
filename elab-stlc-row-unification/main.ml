...
let elab_tm (source : Source_file.t) (tm : Surface.tm) : Core.tm * Core.ty =
  ...
  | unsolved_metas ->
      unsolved_metas |> List.iter (function
        | (pos, `Fun_param) -> print_error source pos "ambiguous function parameter type"
        | (pos, `Fun_body) -> print_error source pos "ambiguous function return type"
        | (pos, `Record_field) -> print_error source pos "ambiguous record field"
        | (pos, `Match_clauses) -> print_error source pos "ambiguous match clauses"
        | (pos, `Pattern_binder) -> print_error source pos "ambiguous pattern binder"
