...
Function application
  $ stlc-fix-records <<< "let f x := x; f 3"
  let f : Int -> Int := fun (x : Int) => x; f 3 : Int
  3

Explicit parameter type
  $ stlc-fix-records <<< "let f (x : Int) := x; f 3"
  let f : Int -> Int := fun (x : Int) => x; f 3 : Int
  3

Explicit return type
  $ stlc-fix-records <<< "let f (x : Int) : Int := x; f 3"
  let f : Int -> Int := fun (x : Int) => x; f 3 : Int
  3

Placeholder types
  $ stlc-fix-records <<< "let f (x : _) : _ := x; f 3"
  let f : Int -> Int := fun (x : Int) => x; f 3 : Int
  3
...
Record literals
  $ stlc-fix-records <<< "{x := 1; y := false}"
  { x := 1; y := false } : { x : Int; y : Bool }
  { x := 1; y := false }

Record projection
  $ stlc-fix-records <<< "{x := 1; y := false}.x"
  { x := 1; y := false }.x : Int
  1
...
Fix2: even/odd
  ...
  > }))
  > EOF
  $ cat even-odd.txt | stlc-fix-records
  fix2
...
Unbound variable
  $ stlc-fix-records <<< "let x := 1; y"
  <input>:1:12: unbound name `y`
  [1]

Mismatched definition type
  $ stlc-fix-records <<< "let x : Bool := 1; x"
...
Mismatched argument
  $ stlc-fix-records <<< "let f x := x + 1; f f"
...
Mismatched argument
  $ stlc-fix-records <<< "let f (x : Bool) := x; f 1"
