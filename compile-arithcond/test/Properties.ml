...
let rec shrink_term =
  ...
  | TreeLang.Eq (n1, n2) ->
      QCheck.Iter.empty (* the subexpressions might not be booleans, so don't shrink to them *)
        <+> (let+ n1' = shrink_term n1 in TreeLang.eq n1' n2)
        <+> (let+ n2' = shrink_term n2 in TreeLang.eq n1 n2')
  | TreeLang.IfThenElse (n1, n2, n3) ->
      ...
      QCheck.Iter.of_list [n2; n3]
        <+> (let+ n1' = shrink_term n1 in TreeLang.if_then_else n1' n2 n3)
        <+> (let+ n2' = shrink_term n2 in TreeLang.if_then_else n1 n2' n3)
        <+> (let+ n3' = shrink_term n3 in TreeLang.if_then_else n1 n2 n3'))
...
let arbitrary_term =
  QCheck.make gen_term
    ~print:(Format.asprintf "%a" TreeLang.pp_term)
    ~shrink:shrink_term

let arbitrary_term_well_typed =
  QCheck.make gen_term_well_typed
    ~print:(Format.asprintf "%a" TreeLang.pp_term)
    ~shrink:shrink_term
...
let compile_correct =
  ...
  QCheck.Test.make ~count:1000
    ...
    arbitrary_term_well_typed
    (fun term ->
      eval_stack (ArithCond.TreeToStack.translate term) = to_stack (eval_tree term))
