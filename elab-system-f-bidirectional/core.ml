...
and pp_atomic_ty ty_names ppf ty =
  match ty with
  | Var ty_index -> Format.fprintf ppf "%a" pp_name (List.nth ty_names ty_index)
...
let rec pp_tm (ty_names : name env) (tm_names : name env) (ppf : Format.formatter) (tm : tm) : unit =
  let rec go_params ty_names tm_names ppf tm =
    match tm with
    | Forall_lit (name, body) ->
...
and pp_atomic_tm ty_names tm_names ppf tm =
  match tm with
  | Var tm_index -> Format.fprintf ppf "%a" pp_name (List.nth tm_names tm_index)
