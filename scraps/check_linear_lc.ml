...
module Check : sig
  ...
  module Ctx : sig
    ...
    val add_binding : string -> Ty.t -> t -> t
    val remove_binding : t -> t
    ...
    let add_binding (x : string) (ty : Ty.t) (ctx : t) : t =
      (x, ty, Zero) :: ctx

    let remove_binding (ctx : t) : t =
      match ctx with
      | (_, _, One) :: ctx -> ctx
      | (x, _, Zero) :: _ -> type_error "unused variable `%s`" x
      | [] -> failwith "bug: no bindings to remove"
  ...
  let rec check (ctx : Ctx.t) (expr : Expr.t) (ty : Ty.t) : Ctx.t =
    ...
    | Expr.Let (x, def, body), body_ty ->
        ...
        let ctx = check (ctx |> Ctx.add_binding x def_ty) body body_ty in
        ctx |> Ctx.remove_binding

    | Expr.Fun_lit (x, body), Ty.Fun (param_ty, body_ty) ->
        let ctx = check (ctx |> Ctx.add_binding x param_ty) body body_ty in
        ctx |> Ctx.remove_binding

    | Expr.Pair_let (x1, x2, pair, body), body_ty ->
        begin match infer ctx pair with
        | ctx, Ty.Pair (ty1, ty2) ->
            let ctx = check (ctx |> Ctx.add_binding x1 ty1 |> Ctx.add_binding x2 ty2) body body_ty in
            ctx |> Ctx.remove_binding |> Ctx.remove_binding
    ...
    | Expr.Either_case (either, (x1, body1), (x2, body2)), body_ty ->
        ...
        | ctx, Ty.Either (ty1, ty2) ->
            let ctx1 = check (ctx |> Ctx.add_binding x1 ty1) body1 body_ty |> Ctx.remove_binding in
            let ctx2 = check (ctx |> Ctx.add_binding x2 ty2) body2 body_ty |> Ctx.remove_binding in
  ...
  and infer (ctx : Ctx.t) (expr : Expr.t) : Ctx.t * Ty.t =
    ...
    | Expr.Let (x, def, body) ->
        let ctx, def_ty = infer ctx def in
        let ctx, body_ty = infer (ctx |> Ctx.add_binding x def_ty) body in
        ctx |> Ctx.remove_binding, body_ty
    ...
    | Expr.Pair_let (x1, x2, pair, body) ->
        begin match infer ctx pair with
        | ctx, Ty.Pair (ty1, ty2) ->
            let ctx, body_ty = infer (ctx |> Ctx.add_binding x1 ty1 |> Ctx.add_binding x2 ty2) body in
            ctx |> Ctx.remove_binding |> Ctx.remove_binding, body_ty
        | _, ty -> type_error "expected pair, found: %t" (Ty.pp ty)
        end
  ...
  let run (type a) (prog : unit -> Ctx.t * a) : (a, string) result =
    match prog () with
    | ctx, x when not (Ctx.is_empty ctx) -> Ok x
    | exception Type_error msg -> Error msg
    (* This will occur if we forgot to call [Ctx.remove_binding] the right number of times. *)
