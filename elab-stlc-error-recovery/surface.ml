...
module Elab : sig
  ...
  and infer_tm (ctx : context) (tm : tm) : Core.tm * Core.ty =
    ...
    | Prim name ->
        ...
        | None ->
            error ctx tm.span "unknown primitive operation `#%s`" name;
            Core.Reported_error, Core.Unknown_type
        end
    ...
    | Infix (`Eq, tm1, ({ span = tm2_span; _ } as tm2)) ->
        ...
        | Some Core.Bool_type -> Core.Fun_app (Fun_app (Prim Prim.Bool_eq, tm1), tm2), Core.Bool_type
        | Some Core.Int_type -> Core.Fun_app (Fun_app (Prim Prim.Int_eq, tm1), tm2), Core.Bool_type
        | Some ty ->
            error ctx tm.span
    ...
    | Prefix (`Neg, tm) ->
        ...
        Fun_app (Prim Prim.Int_neg, tm), Core.Int_type
